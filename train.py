...
"""
logger = create_logger('global_logger', 'log.txt')
logger.info('CNNGeometric training script')
...
parser.add_argument('--random-sample', type=str_to_bool, nargs='?', const=True, default=True, help='sample random transformations')
...
for epoch in range(1, args.num_epochs+1):
    train_loss = train(epoch,model,loss,optimizer,dataloader,pair_generation_tnf,log_interval=50,logger=logger)
    test_loss = test(model,loss,dataloader_test,pair_generation_tnf,logger=logger)
